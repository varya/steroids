  ...
  (function(grunt){
    ...
    grunt.registerTask('steroids-compile-coffeescripts', "Compiles coffeescripts from app/models/* app/controllers/* and vendor/appgyver/*", function() {
      [appModelsDirectory, appControllersDirectory].forEach( function(directory) {
        grunt.file.expand(path.join(directory, "**", "*.coffee")).forEach( function(filePath) {
          compileCoffee(filePath, appDirectory);
        });
      });

      grunt.file.expand(path.join(vendorDirectory, "**", "*.coffee")).forEach( function(filePath) {
        compileCoffee(filePath, vendorDirectory, "vendor");
      });
    });

    grunt.registerTask('steroids-concat', 'Concatenate steroids project files in dist/', function() {
      var jsArr=[];

      grunt.file.expand(grunt.file.expandFiles(path.join(buildModelsDirectory, "**", "*"))).forEach( function(filePath) {
    ...
    grunt.registerTask('steroids-copy-vendor', 'Copy vendor/ to dist/vendor', function() {
      grunt.file.expandFiles(path.join(vendorDirectory,"**","*")).forEach( function(filePathPart) {
        if (!/\.coffee$/.test(path.basename(filePathPart))) {
          var filePath = path.resolve(filePathPart);
          var buildFilePath = path.resolve(filePathPart.replace("vendor"+path.sep, "dist"+path.sep+"vendor"+path.sep));
    ...
    grunt.registerTask('steroids-remove-dist-models', 'Remove single model files from build dir', function() {
      var buildModelFiles = grunt.file.expandFiles(path.join(buildModelsDirectory, "**", "*"));

      buildModelFiles.forEach(function(filePath) {
        if (path.basename(filePath) !== "models.js")
          fs.unlink(filePath);
      });
    });
    ...
    grunt.registerTask('steroids-generate-views', 'HTML files from app/layouts/application & app/**/* files', function() {
      ...
      grunt.file.expandDirs(path.join(appViewsDirectory, "*")).forEach( function(dirPath) {
      ...
      viewDirectories.forEach( function(viewDir) {
        ...
        grunt.file.expand(path.join(viewDir, "**", "*")).forEach( function(filePathPart) {
          var filePath = path.resolve(filePathPart);
          var buildFilePath = path.resolve(filePathPart.replace("app"+path.sep, "dist"+path.sep));
          ...
          var yieldedFile = grunt.util._.template(applicationLayoutFile.toString())({
            yield: yieldObj
          });
